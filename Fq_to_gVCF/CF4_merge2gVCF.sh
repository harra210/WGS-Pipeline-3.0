...
#PREFIX="chr"
#
#printf "%s\n" "${INT[@]/#/$PREFIX}" &> "$tmpdir"/BaseRecalibrator_1pass.intervals
#printf "%s\n" "${INT_2[@]/#/$PREFIX}" &> "$tmpdir"/BaseRecalibrator_2pass.intervals
#
#BQSR_INT1=""$tmpdir"/BaseRecalibrator_1pass.intervals"
#BQSR_INT2=""$tmpdir"/BaseRecalibrator_2pass.intervals"
#
cd $FILE_DIR
#
...
for ((i = 0; i < ${#lsdir[@]}; i++))
do
        echo "cd "${lsdir[$i]}"; gatk --java-options \"-Xmx16g -XX:ParallelGCThreads=4\" BaseRecalibrator -bqsr-baq-gap-open-penalty 40.0 -R "$CanFam4" --tmp-dir /lscratch/\$SLURM_JOB_ID -I dedup_"${sample[$i]}".bam --known-sites "$knownsite" -L "$BQSR_INT1" -O "${sample[$i]}"_firstpass_recal.table; gatk --java-options \"-Xmx16g -XX:ParallelGCThreads=4\" ApplyBQSR -R "$CanFam4" --tmp-dir /lscratch/\$SLURM_JOB_ID -I dedup_"${sample[$i]}".bam -bqsr "${sample[$i]}"_firstpass_recal.table -O "${sample[$i]}"_BQSR.bam" >> CF4_BQSR.swarm
done
#
head CF4_BQSR.swarm
read -p "Does the formatting of the swarmfile appear correct? (yes or no) " promptA
...
for ((i = 0; i < ${#lsdir[@]}; i++))
do
	echo "cd "${lsdir[$i]}"; gatk --java-options \"-Xmx16g -XX:ParallelGCThreads=4\" BaseRecalibrator -bqsr-baq-gap-open-penalty 40.0 -R "$CanFam4" --tmp-dir /lscratch/\$SLURM_JOB_ID -I "${sample[$i]}"_BQSR.bam --known-sites "$knownsite" -L "$BQSR_INT2" -O "${sample[$i]}"_secondpass_recal.table; gatk AnalyzeCovariates --tmp-dir /lscratch/\$SLURM_JOB_ID -before "${sample[$i]}"_firstpass_recal.table -after "${sample[$i]}"_secondpass_recal.table -plots "${sample[$i]}"_Covariates.pdf" >> CF4_BQSR2pass_Covariates.swarm
done
#
